...
class RewardMachine:
    ...
    def get_reward(self,u1,u2,s1=None,a=None,s2=None):
        if u1 in self.delta_r and u2 in self.delta_r[u1]:
            return self.delta_r[u1][u2].get_reward(s1,a,s2)###n.delta_r[key][value] is an object of reward function class, so we can use .get_reward
    ...
    def _load_reward_machine(self, file):
        """ This text specifies the reward machine (which is an FSM)
        Example:
            0                  # initial state
    ...
    def _add_state(self, u_list):
        for u in u_list:#iterate through the u_list 
            if u not in self.U:#if u is not in self.U, 
                self.U.append(u)#Then add it to self.U

    def _add_transition(self, u1, u2, dnf_formula, reward_function):# starting state, next state, boolean formula for the state transition, reward function at this state(u1)
        # Adding machine state
        self._add_state([u1,u2])#add the states to self.U
        # Adding state-transition to delta_u
        if u1 not in self.delta_u:#if this state's transition info is not in self.delta.u
            self.delta_u[u1] = {} #add an empty dict
        if u2 not in self.delta_u[u1]: #if u2 has not been recorded at the next state after u1
           self.delta_u[u1][u2] = dnf_formula #go to u1, make a key u2, make the value the dnf formula
        else:#if u2 is already there, then add this dnf formula to the formula there
